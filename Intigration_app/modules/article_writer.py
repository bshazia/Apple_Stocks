...
def extract(topic=None, file=None, url=None):
    try:
        if not topic:
        ...
        if "error" in original_topic_language.lower():
            return {"error": original_topic_language}, 500

        if file:
            # Handle the file case (PDF)
            try:
                text = text_pdf(file)
                ...
                if not is_topic_intext(text, topic):
                    return {"error": "The provided topic is not mentioned in the document."}, 400
                ...
                return {"error": f"Error processing PDF file: {str(e)}"}, 500
            ...
            try:
                ...
                if original_topic_language != text_language:
                    topic = translate(topic, text_language)
                if not is_topic_intext(text, topic):
                    return {"error": "The provided topic is not mentioned on the website."}, 400
